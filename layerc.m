function [T,RD]=layerc(cp,rho,dz,nt,dt,np,dp,p0,norm,mul)
% LAYERCODE 	Compute the  response of 
%		an 1D acoustic medium
% 
% syntax: [T,R]=layercode(Cp,Rho,dz,nt,dt,np,dp,p0,norm)
%
%
...
RD = [Rd(1:nt/2,:) ;real(Rd(nt/2+1,:)) ;conj(Rd(nt/2:-1:2,:))];
RD         =    real(ifft(conj(RD)));
